...
export class BuiltInBlockingController {
    constructor(private readonly builtInBlockingService: BuiltInBlockingService) { }
     ...
     */
    @Get(':nodeId')
    async getSnapshot(@Param('nodeId') nodeId: string): Promise<BuiltInBlockingSnapshot> {
     ...
     */
    @Get(':nodeId/settings')
    async getSettings(@Param('nodeId') nodeId: string): Promise<BlockingSettings> {
    ...
    async forceUpdate(@Param('nodeId') nodeId: string): Promise<BlockingZoneOperationResult> {
    ...
    async temporaryDisable(
        ...
        if (body?.minutes === undefined || typeof body.minutes !== 'number' || body.minutes < 0) {
            throw new BadRequestException('minutes is required and must be a non-negative number');
        }
        return this.builtInBlockingService.temporaryDisableBlocking(nodeId, body.minutes);
    }
    ...
    async reEnable(@Param('nodeId') nodeId: string): Promise<BlockingZoneOperationResult> {
    ...
    async addAllowed(
        ...
        return this.builtInBlockingService.addAllowedZone(nodeId, { domain: body.domain.trim() });
    }
    ...
    async flushAllowed(@Param('nodeId') nodeId: string): Promise<BlockingZoneOperationResult> {
    ...
    async exportAllowed(@Param('nodeId') nodeId: string, @Res() res: Response): Promise<void> {
        const content = await this.builtInBlockingService.exportAllowedZones(nodeId);
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
        res.setHeader('Content-Disposition', `attachment; filename="allowed-${nodeId}.txt"`);
        res.send(content);
    }
    ...
    async addBlocked(
        ...
        return this.builtInBlockingService.addBlockedZone(nodeId, { domain: body.domain.trim() });
    }
    ...
    async flushBlocked(@Param('nodeId') nodeId: string): Promise<BlockingZoneOperationResult> {
    ...
    async exportBlocked(@Param('nodeId') nodeId: string, @Res() res: Response): Promise<void> {
        const content = await this.builtInBlockingService.exportBlockedZones(nodeId);
        res.setHeader('Content-Type', 'text/plain; charset=utf-8');
        res.setHeader('Content-Disposition', `attachment; filename="blocked-${nodeId}.txt"`);
        res.send(content);
    }
}
