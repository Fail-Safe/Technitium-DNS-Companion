import { BadRequestException, Body, Controller, Delete, Get, Param, Post, Query, Res, UseInterceptors } from '@nestjs/common';
...
export class TechnitiumController {
  ...
  @Get(':nodeId/logs')
  getQueryLogs(@Param('nodeId') nodeId: string, @Query() query: Record<string, string | string[]>) {
    const filters = this.normalizeQueryLogFilters(query);
  ...
  getDhcpScope(@Param('nodeId') nodeId: string, @Param('scopeName') scopeName: string) {
  ...
  bulkSyncDhcpScopes(@Body() body: DhcpBulkSyncRequest) {
    if (!body || !body.sourceNodeId || !body.targetNodeIds || body.targetNodeIds.length === 0) {
  ...
  private normalizeQueryLogFilters(
    ...
    if (entriesPerPageRaw !== undefined) {
      ...
      if (!Number.isFinite(parsed) || parsed < 1) {
        throw new BadRequestException('"entriesPerPage" must be a positive integer.');
      }
    ...
    if (descendingOrderRaw !== undefined) {
      if (/^(true|1)$/i.test(descendingOrderRaw)) {
        ...
        throw new BadRequestException('"descendingOrder" must be "true" or "false".');
      }
    }
    ...
    if (deduplicateDomainsRaw !== undefined) {
      if (/^(true|1)$/i.test(deduplicateDomainsRaw)) {
        ...
        throw new BadRequestException('"deduplicateDomains" must be "true" or "false".');
      }
    }
    ...
    if (disableCacheRaw !== undefined) {
      if (/^(true|1)$/i.test(disableCacheRaw)) {
        ...
        throw new BadRequestException('"disableCache" must be "true" or "false".');
      }
    }
