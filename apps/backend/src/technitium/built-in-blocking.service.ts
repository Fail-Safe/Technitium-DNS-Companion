...
import type { TechnitiumApiResponse, TechnitiumNodeSummary } from './technitium.types';
import type {
    BlockingZoneEntry,
...
export class BuiltInBlockingService {
    ...
    async getOverview(): Promise<BuiltInBlockingOverview> {
        ...
        const aggregate: BuiltInBlockingMetrics = healthySnapshot?.metrics ?? this.emptyMetrics();
    ...
    async getBlockingStatus(): Promise<BlockingStatusOverview> {
        ...
        for (const summary of summaries) {
            try {
                ...
                if (advancedBlockingInstalled) {
                    try {
                        ...
                        if (abOverview.status === 'ok' && abOverview.response?.config) {
                            const config = JSON.parse(abOverview.response.config);
                            advancedBlockingEnabled = config.enableBlocking ?? false;
                        }
                    } catch {
                        // Advanced Blocking config not accessible, assume not enabled
                        this.logger.warn(`Could not get Advanced Blocking config for node "${summary.id}"`);
                    }
                }
                ...
                this.logger.warn(`Failed to get blocking status for node "${summary.id}":`, error);
                nodeStatuses.push({
                    nodeId: summary.id,
                    nodeName: summary.name || summary.id,
    ...
    async listAllowedZones(
        ...
        if (params?.domain) {
            ...
            filteredDomains = allDomains.filter((d) => d.toLowerCase().includes(searchTerm));
        }
        ...
        const totalPages = entriesPerPage > 0 ? Math.ceil(totalEntries / entriesPerPage) : 1;
     ...
     */
    async flushAllowedZones(nodeId: string): Promise<BlockingZoneOperationResult> {
     ...
     */
    async flushBlockedZones(nodeId: string): Promise<BlockingZoneOperationResult> {
    ...
    async updateBlockingSettings(
        ...
        if (settings.customBlockingAddresses !== undefined && settings.customBlockingAddresses !== null) {
            params.customBlockingAddresses = settings.customBlockingAddresses.join(',');
        }

        if (settings.blockListUrls !== undefined && settings.blockListUrls !== null) {
            params.blockListUrls = settings.blockListUrls.join(',');
        }

        if (settings.blockListUrlUpdateIntervalHours !== undefined) {
            // Note: Technitium API uses 'blockListUpdateIntervalHours' (no 'Url')
            params.blockListUpdateIntervalHours = settings.blockListUrlUpdateIntervalHours;
        }
        ...
        try {
            ...
            await this.technitiumService.executeAction<TechnitiumApiResponse<unknown>>(nodeId, {
            ...
            this.logger.error(`Failed to update settings on node "${nodeId}":`, error);
            return {
                success: false,
     ...
     */
    async forceBlockListUpdate(nodeId: string): Promise<BlockingZoneOperationResult> {
        try {
            await this.technitiumService.executeAction<TechnitiumApiResponse<unknown>>(nodeId, {
                method: 'GET',
            ...
            this.logger.error(`Failed to force block list update on node "${nodeId}":`, error);
            return {
                success: false,
                message: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    async temporaryDisableBlocking(
        nodeId: string,
        minutes: number,
    ): Promise<{ success: boolean; temporaryDisableBlockingTill?: string; message?: string }> {
        try {
            const response = await this.technitiumService.executeAction<
            ...
            return {
                success: true,
                temporaryDisableBlockingTill: response.response.temporaryDisableBlockingTill,
            };
        } catch (error) {
            this.logger.error(`Failed to temporarily disable blocking on node "${nodeId}":`, error);
            return {
                success: false,
                message: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    async reEnableBlocking(nodeId: string): Promise<BlockingZoneOperationResult> {
        try {
            ...
            this.logger.error(`Failed to re-enable blocking on node "${nodeId}":`, error);
            return {
                success: false,
                message: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    private async loadSnapshot(summary: TechnitiumNodeSummary): Promise<BuiltInBlockingSnapshot> {
        ...
        try {
            ...
            return {
                ...base,
                metrics: {
                    allowedCount: allowedResponse.totalEntries ?? allowedResponse.domains.length,
                    blockedCount: blockedResponse.totalEntries ?? blockedResponse.domains.length,
            ...
            this.logger.warn(`Failed to load snapshot for node "${summary.id}":`, error);
            return {
                ...base,
    ...
    private async listZones(
        ...
        const domains: BlockingZoneEntry[] = (data.zones ?? []).map((zone: string) => {
    ...
    private async getAllDomainsRecursive(
        ...
        if (data.zones && data.zones.length > 0) {
            for (const zone of data.zones) {
                ...
                const fullDomain = parentDomain ? `${zone}.${parentDomain}` : zone;
                ...
                if (subDomains.length > 0) {
                    ...
                    if (checkResponse.status === 'ok' && checkResponse.response?.records && checkResponse.response.records.length > 0) {
    ...
    private async addZone(
        ...
        try {
            ...
            return { success: true, message: `Domain "${params.domain}" added to ${type} list` };
        } catch (error) {
            this.logger.error(`Failed to add domain to ${type} list on node "${nodeId}":`, error);
            return {
                success: false,
                message: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    private async deleteZone(
        ...
        try {
            ...
            return { success: true, message: `Domain "${params.domain}" removed from ${type} list` };
        } catch (error) {
            this.logger.error(`Failed to delete domain from ${type} list on node "${nodeId}":`, error);
            return {
                success: false,
                message: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    private async flushZones(
        ...
        try {
            ...
            this.logger.error(`Failed to flush ${type} zones on node "${nodeId}":`, error);
            return {
                success: false,
                message: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    private async importZones(
        ...
        try {
            const listUrl = Array.isArray(params.listUrl) ? params.listUrl.join(',') : params.listUrl;
            const queryParams = new URLSearchParams();
            ...
            return { success: true, message: `Imported domains to ${type} list from URL(s)` };
        } catch (error) {
            this.logger.error(`Failed to import ${type} zones on node "${nodeId}":`, error);
            return {
                success: false,
                message: error instanceof Error ? error.message : 'Unknown error',
            };
    ...
    private async exportZones(nodeId: string, type: 'allowed' | 'blocked'): Promise<string> {
        // Export returns plain text, not JSON
        const response = await this.technitiumService.executeAction<string>(nodeId, {
            method: 'GET',
            url: `/api/${type}/export`,
            params: {},
        });
