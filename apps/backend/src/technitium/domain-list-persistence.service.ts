...
export class DomainListPersistenceService {
    ...
    constructor() {
        // Use /data directory for Docker volumes, fallback to local cache
        this.cacheBaseDir = process.env.CACHE_DIR || '/data/domain-lists-cache';
        this.logger.log(`Cache directory: ${this.cacheBaseDir}`);
    }
    ...
    async initialize(): Promise<void> {
        try {
            ...
            this.logger.log(`Cache directory initialized: ${this.cacheBaseDir}`);
        } catch (error) {
    ...
    private getCacheFilePaths(nodeId: string, hash: string) {
        ...
        const dataFile = join(nodeDir, `${hash}.data${this.useCompression ? '.gz' : ''}`);
     ...
     */
    async loadCache(nodeId: string, hash: string): Promise<PersistedCache | null> {
        try {
            const { metadataFile, dataFile } = this.getCacheFilePaths(nodeId, hash);
     ...
     */
    async getCacheMetadata(nodeId: string, hash: string): Promise<CacheMetadata | null> {
        try {
            ...
            this.logger.error(`Failed to get cache metadata for ${nodeId}/${hash}:`, error);
            return null;
        }
    }
