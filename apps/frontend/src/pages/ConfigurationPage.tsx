...
import { faUsers, faPencil, faTrash, faInfoCircle, faPlus, faXmark, faChevronUp, faList, faSearch, faCode, faBan, faSquareCheck, faSquareMinus, faCheck, faCircle, faGripVertical, faMinus, faExclamationTriangle } from '@fortawesome/free-solid-svg-icons';
import './ConfigurationPage.css';

type TabMode = 'domain-management' | 'group-management' | 'list-management' | 'sync';
...
export function ConfigurationPage() {
    ...
    const [hasUnsavedListSourcesChanges, setHasUnsavedListSourcesChanges] = useState(false);
    const [hasUnsavedDomainChanges, setHasUnsavedDomainChanges] = useState(false);

    // Combined unsaved changes check
    const hasAnyUnsavedChanges = hasUnsavedGroupChanges || hasUnsavedListSourcesChanges || hasUnsavedDomainChanges;
    ...
    const [domainMatchDetails, setDomainMatchDetails] = useState<Array<{
        type: string;
        source?: string;
        groupName?: string;
        groups?: string[];
    ...
    const [activeDomainType, setActiveDomainType] = useState<'blocked' | 'allowed' | 'blockedRegex' | 'allowedRegex'>('blocked');
    const [expandedGroups, setExpandedGroups] = useState<Set<string>>(new Set());

    // Domain Management tab: Staged changes tracking
    const [testStagedConfig, setTestStagedConfig] = useState<AdvancedBlockingConfig | null>(null);
    const [testPendingChanges, setTestPendingChanges] = useState<Array<{ type: 'added' | 'removed' | 'modified', category: string, description: string }>>([]);
    const [showTestChangesSummary, setShowTestChangesSummary] = useState(false);

    // Domain Management tab: Edit/Delete modals
    const [editingDomain, setEditingDomain] = useState<{ domain: string, groups: string[] } | null>(null);
    const [editDomainInput, setEditDomainInput] = useState('');
    const [deletingDomain, setDeletingDomain] = useState<{ domain: string, groups: string[] } | null>(null);
    ...
    const someMissing = missingNodes.length > 0 && missingNodes.length < nodes.length;
    ...
    useEffect(() => {
        ...
        for (const groupName of allGroupNames) {
            ...
            if (!group1 && group2) {
                ...
                const isDifferentSettings =
                    ...
                    !compareSettingValues(group1.blockAsNxDomain, group2.blockAsNxDomain) ||
                    !compareSettingValues(group1.allowTxtBlockingReport, group2.allowTxtBlockingReport) ||
                    !compareSettingValues(group1.blockingAddresses, group2.blockingAddresses);
        ...
        if (node1.config!.blockListUrlUpdateIntervalHours !== node2.config!.blockListUrlUpdateIntervalHours) {
            configDiffsCount++;
        }
        ...
        if (localKeys1.length !== localKeys2.length ||
            !localKeys1.every((key, i) => key === localKeys2[i] && localMappings1[key] === localMappings2[key])) {
            configDiffsCount++;
        }
        ...
        if (networkKeys1.length !== networkKeys2.length ||
            !networkKeys1.every((key, i) => key === networkKeys2[i] && networkMappings1[key] === networkMappings2[key])) {
            configDiffsCount++;
        }

        setSyncChangeCount(differentCount + onlyNode1Count + onlyNode2Count + configDiffsCount);
    }, [advancedBlocking]);
    ...
    const handleSyncConfig = useCallback(
        async (_sourceNodeId: string, targetNodeId: string, config: AdvancedBlockingConfig) => {
    ...
    const selectedNodeConfig = advancedBlocking?.nodes?.find(
        (n) => n.nodeId === selectedNodeId,
    )?.config;

    const availableNodes = advancedBlocking?.nodes ?? [];
    ...
    const handleTabChange = useCallback(
        (newTab: TabMode) => {
            ...
            } else if (activeTab === 'list-management' && hasUnsavedListSourcesChanges) {
                hasUnsaved = true;
    ...
    const handleCheckDomain = useCallback(async (domainOverride?: string) => {
    ...
    const handleRemoveFromGroup = useCallback((domain: string, groupName: string) => {
        ...
        const arrayKey = activeDomainType === 'blocked' ? 'blocked' :
            activeDomainType === 'allowed' ? 'allowed' :
                activeDomainType === 'blockedRegex' ? 'blockedRegex' : 'allowedRegex';
    ...
    const handleDrop = async (e: React.DragEvent, groupName: string | 'ALL_GROUPS') => {
        e.preventDefault();
        ...
        const updatedConfig = JSON.parse(JSON.stringify(testStagedConfig)) as AdvancedBlockingConfig;

        // Determine which groups to add to
        const targetGroups = groupName === 'ALL_GROUPS'
        ...
        if (groupsModified.length > 0) {
            ...
            // Track the change
            const changeDesc = groupName === 'ALL_GROUPS'
            ...
            setTestPendingChanges(prev => [...prev, {
                type: 'added',
                category: activeDomainType,
    ...
    useEffect(() => {
        if (activeTab === 'domain-management' && searchedDomain && testStagedConfig && domainMatchDetails.length > 0) {
    ...
    const allDomainsForType = useMemo(() => {
        ...
        const config = (activeTab === 'domain-management' && testStagedConfig) ? testStagedConfig : selectedNodeConfig;
        ...
        config.groups.forEach(group => {
            let domains: string[] = [];
            switch (activeDomainType) {
                case 'blocked': domains = group.blocked || []; break;
                case 'allowed': domains = group.allowed || []; break;
                case 'blockedRegex': domains = group.blockedRegex || []; break;
                case 'allowedRegex': domains = group.allowedRegex || []; break;
            }
    ...
    const getGroupsForDomain = useCallback((domain: string): string[] => {
        // Use testStagedConfig on domain-management tab to show staged changes, otherwise use selectedNodeConfig
        const config = (activeTab === 'domain-management' && testStagedConfig) ? testStagedConfig : selectedNodeConfig;
        if (!config?.groups) return [];
    ...
    const getDomainsForGroupByType = useCallback((groupName: string): string[] => {
        // Use testStagedConfig on domain-management tab to show staged changes, otherwise use selectedNodeConfig
        const config = (activeTab === 'domain-management' && testStagedConfig) ? testStagedConfig : selectedNodeConfig;
        ...
        switch (activeDomainType) {
            case 'blocked': return group.blocked || [];
            case 'allowed': return group.allowed || [];
            case 'blockedRegex': return group.blockedRegex || [];
            case 'allowedRegex': return group.allowedRegex || [];
        }
    }, [selectedNodeConfig, testStagedConfig, activeDomainType, activeTab]);
    ...
    }, [testStagedConfig, selectedNodeId, saveAdvancedBlockingConfig, reloadAdvancedBlocking]);
    ...
    const handleEditDomain = useCallback((domain: string) => {
            ...
            <section ref={pullToRefresh.containerRef} className="configuration">
                <header className="configuration__header">
                    <div>
                        <h1>DNS Filtering</h1>
                        <p>
                            Configure DNS filtering rules, groups, and sync policies across nodes.
                        </p>
                    </div>
                                                            ...
                                                            <div className={`alert-box ${isBlocked ? 'alert-box--danger' : 'alert-box--success'}`}>
                                                                ...
                                                                {domainMatchDetails.length > 0 && isBlocked && (
                                                                    <div className="match-details match-details--danger">
                                                                        <p className="match-details__title">Match Details:</p>
                                                                        <ul className="match-details__list">
                                                                            {(() => {
                                                                                ...
                                                                                const uniqueMatches = domainMatchDetails
                                                                                    .filter(m => m.type !== 'manual-allowed')
                                                                                    .reduce((acc, match) => {
                                                                                ...
                                                                                return Array.from(uniqueMatches.values()).map((match, idx) => (
                                                                                    <li key={idx} style={{ marginBottom: '0.5rem' }}>
                                                                                        <div style={{ fontStyle: 'italic', marginBottom: '0.25rem' }}>
                                                                                            {match.type}
                                                                                            {match.matchedDomain && ` (matched: ${match.matchedDomain})`}
                                                                                            {match.matchedPattern && ` (pattern: ${match.matchedPattern})`}
                                                                                        </div>
                                                                                        {match.source && (
                                                                                            <div style={{ fontSize: '0.75rem', opacity: 0.8, paddingLeft: '0.5rem', borderLeft: '2px solid currentColor' }}>
                                                                                                Source: {match.source}
                                                                                            </div>
                                                                                        )}
                                                                                    </li>
                                                                        ...
                                                                        {dragOverGroup === groupName && (
                                                                            <span className="group-drop-zone__action-icon">
                                                                                {dragSourceGroup === groupName ? (
                                                                                    <FontAwesomeIcon icon={faXmark} className="group-drop-zone__action-icon--remove" />
                                                                                ) : (
                                                                                    <FontAwesomeIcon icon={faPlus} className="group-drop-zone__action-icon--add" />
                                                                                )}
                                                                            </span>
                                                                        )}
                                                                    </div>
                ...
                >
                    ...
                    >
                        ...
                        <p className="modal__description">
                            Domain is in {editingDomain.groups.length} group(s): <strong>{editingDomain.groups.join(', ')}</strong>
                        </p>
                        <input
                            type="text"
                ...
                >
                    ...
                    >
                        <h3 className="modal__title modal__title--danger">
                            <FontAwesomeIcon icon={faExclamationTriangle} style={{ fontSize: '1.1em' }} />
                            Delete Domain
                        </h3>
                        <p className="modal__description">
                            Are you sure you want to delete <code className="modal__code">{deletingDomain.domain}</code> from <strong>{deletingDomain.groups.length} group(s)</strong>?
                        </p>
