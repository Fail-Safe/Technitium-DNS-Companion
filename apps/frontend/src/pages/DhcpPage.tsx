...
import { faPlus, faMinus, faPencil, faClipboard } from '@fortawesome/free-solid-svg-icons';
...
const arraysEqual = (first: string[] | undefined, second: string[] | undefined): boolean => {
    const a = first ?? [];
    const b = second ?? [];
...
const describeGenericOptionValue = (entry: GenericOptionDraft): GenericOptionPreview => {
...
const buildStaticRouteDraft = (route?: TechnitiumDhcpStaticRoute): StaticRouteDraft => ({
    id: createDraftId(),
...
const buildVendorInfoDraft = (info?: TechnitiumDhcpVendorInfo): VendorInfoDraft => ({
    id: createDraftId(),
    identifier: info?.identifier ?? '',
    information: info?.information ?? '',
});

const buildGenericOptionDraft = (option?: TechnitiumDhcpGenericOption): GenericOptionDraft => {
...
const buildExclusionDraft = (range?: TechnitiumDhcpExclusionRange): ExclusionDraft => ({
    id: createDraftId(),
    startingAddress: range?.startingAddress ?? '',
    endingAddress: range?.endingAddress ?? '',
});

const buildReservedLeaseDraft = (lease?: TechnitiumDhcpReservedLease): ReservedLeaseDraft => ({
    id: createDraftId(),
...
const sanitizeStaticRoutes = (routes: StaticRouteDraft[]): SanitizedCollectionResult<TechnitiumDhcpStaticRoute> => {
...
const sanitizeVendorInfo = (entries: VendorInfoDraft[]): SanitizedCollectionResult<TechnitiumDhcpVendorInfo> => {
    let hasPartial = false;
...
const sanitizeGenericOptions = (entries: GenericOptionDraft[]): SanitizedCollectionResult<TechnitiumDhcpGenericOption> => {
    let hasPartial = false;
    ...
    entries.forEach((entry) => {
        ...
        if (parsedCode === undefined || !Number.isFinite(parsedCode) || parsedCode < 0) {
            invalidCode = true;
            return;
        }
...
const sanitizeExclusions = (entries: ExclusionDraft[]): SanitizedCollectionResult<TechnitiumDhcpExclusionRange> => {
    let hasPartial = false;
...
const sanitizeReservedLeases = (entries: ReservedLeaseDraft[]): SanitizedCollectionResult<TechnitiumDhcpReservedLease> => {
    let hasPartial = false;
    ...
    entries.forEach((entry) => {
        ...
        const anyValues = [hostName, hardwareAddress, address, comments].some((value) => value.length > 0);
...
const formatLeaseDuration = (scope: TechnitiumDhcpScope | undefined): string => {
    if (!scope) {
        return '—';
    }
    ...
    if (scope.leaseTimeDays && scope.leaseTimeDays > 0) {
        parts.push(`${scope.leaseTimeDays} day${scope.leaseTimeDays === 1 ? '' : 's'}`);
    }

    if (scope.leaseTimeHours && scope.leaseTimeHours > 0) {
        parts.push(`${scope.leaseTimeHours} hour${scope.leaseTimeHours === 1 ? '' : 's'}`);
    }

    if (scope.leaseTimeMinutes && scope.leaseTimeMinutes > 0) {
        parts.push(`${scope.leaseTimeMinutes} minute${scope.leaseTimeMinutes === 1 ? '' : 's'}`);
    }
...
export function DhcpPage() {
    ...
    const [scopeCountByNode, setScopeCountByNode] = useState<Map<string, number>>(new Map());
    ...
    const [selectedScopeName, setSelectedScopeName] = useState<string | undefined>();
    const [scopeDetailState, setScopeDetailState] = useState<LoadState>('idle');
    const [scopeDetailError, setScopeDetailError] = useState<string | undefined>();
    const [currentScope, setCurrentScope] = useState<TechnitiumDhcpScope | undefined>();
    const [detailCache, setDetailCache] = useState<Map<string, TechnitiumDhcpScope>>(() => new Map());
    ...
    const [newScopeNameTouched, setNewScopeNameTouched] = useState<boolean>(false);
    ...
    const [draftDomainSearchList, setDraftDomainSearchList] = useState<string>('');
    ...
    const [draftAllowOnlyReserved, setDraftAllowOnlyReserved] = useState<boolean>(false);
    const [draftOfferDelay, setDraftOfferDelay] = useState<string>('');
    const [draftPingCheckEnabled, setDraftPingCheckEnabled] = useState<boolean>(false);
    ...
    const [draftUseThisDnsServer, setDraftUseThisDnsServer] = useState<boolean>(false);
    ...
    const [draftStaticRoutes, setDraftStaticRoutes] = useState<StaticRouteDraft[]>([]);
    const [draftVendorInfo, setDraftVendorInfo] = useState<VendorInfoDraft[]>([]);
    const [draftCapwapControllers, setDraftCapwapControllers] = useState<string>('');
    const [draftTftpServers, setDraftTftpServers] = useState<string>('');
    const [draftGenericOptions, setDraftGenericOptions] = useState<GenericOptionDraft[]>([]);
    const [draftExclusions, setDraftExclusions] = useState<ExclusionDraft[]>([]);
    const [draftReservedLeases, setDraftReservedLeases] = useState<ReservedLeaseDraft[]>([]);
    ...
    const [bulkSyncResult, setBulkSyncResult] = useState<DhcpBulkSyncResult | null>(null);
    const [showBulkSyncResults, setShowBulkSyncResults] = useState(false);

    // Bulk sync form state (inline in tab)
    const [bulkSyncSourceNodeId, setBulkSyncSourceNodeId] = useState<string>(() => nodes[0]?.id ?? '');
    const [bulkSyncTargetNodeIds, setBulkSyncTargetNodeIds] = useState<string[]>([]);
    const [bulkSyncStrategy, setBulkSyncStrategy] = useState<DhcpBulkSyncStrategy>('skip-existing');
    const [bulkSyncEnableOnTarget, setBulkSyncEnableOnTarget] = useState<boolean>(false);

    // Bulk sync preview state
    const [bulkSyncSourceScopes, setBulkSyncSourceScopes] = useState<TechnitiumDhcpScopeSummary[]>([]);
    ...
    const bulkSyncCanStart = useMemo(() => {
        return bulkSyncSourceNodeId && bulkSyncTargetNodeIds.length > 0 && !bulkSyncInProgress;
    ...
    const syncDraftWithScope = (scope?: TechnitiumDhcpScope) => {
        ...
        setDraftDnsTtl(
            scope.dnsTtl !== undefined && scope.dnsTtl !== null ? String(scope.dnsTtl) : '',
        );
        ...
        setDraftStaticRoutes((scope.staticRoutes ?? []).map((route) => buildStaticRouteDraft(route)));
        setDraftVendorInfo((scope.vendorInfo ?? []).map((entry) => buildVendorInfoDraft(entry)));
        setDraftCapwapControllers((scope.capwapAcIpAddresses ?? []).join('\n'));
        setDraftTftpServers((scope.tftpServerAddresses ?? []).join('\n'));
        setDraftGenericOptions((scope.genericOptions ?? []).map((option) => buildGenericOptionDraft(option)));
        setDraftExclusions((scope.exclusions ?? []).map((entry) => buildExclusionDraft(entry)));
        setDraftReservedLeases((scope.reservedLeases ?? []).map((entry) => buildReservedLeaseDraft(entry)));
        setDraftBlockLocallyAdministered(scope.blockLocallyAdministeredMacAddresses ?? false);
    ...
    const serializeDraftToScope = useCallback((): Partial<TechnitiumDhcpScope> | undefined => {
        ...
        return {
            ...
            startingAddress: draftStartingAddress.trim() || currentScope.startingAddress,
            ...
            pingCheckTimeout: draftPingCheckEnabled ? parseLeaseComponent(draftPingTimeout) : undefined,
            pingCheckRetries: draftPingCheckEnabled ? parseLeaseComponent(draftPingRetries) : undefined,
            ...
            dnsServers: draftUseThisDnsServer ? undefined : splitListInput(draftDnsServers),
    ...
    const pendingChanges = useMemo(() => {
        ...
        const baselineScope = JSON.parse(baselineJson) as Partial<TechnitiumDhcpScope>;
        ...
        draftReserved.forEach(draftRes => {
            ...
            if (baselineRes && JSON.stringify(baselineRes) !== JSON.stringify(draftRes)) {
                ...
                if (baselineRes.address !== draftRes.address) {
                    changedFields.push(`IP: ${baselineRes.address} → ${draftRes.address}`);
                }
                if (baselineRes.hardwareAddress !== draftRes.hardwareAddress) {
                    changedFields.push(`MAC: ${baselineRes.hardwareAddress} → ${draftRes.hardwareAddress}`);
                }
                if ((baselineRes.hostName || '') !== (draftRes.hostName || '')) {
                    changedFields.push(`Name: ${baselineRes.hostName || '(none)'} → ${draftRes.hostName || '(none)'}`);
                }
                if ((baselineRes.comments || '') !== (draftRes.comments || '')) {
                    changedFields.push(`Comments: ${baselineRes.comments || '(none)'} → ${draftRes.comments || '(none)'}`);
                }
        ...
        const baselineExclusionStrs = new Set(baselineExclusions.map(e => `${e.startingAddress}-${e.endingAddress}`));
        const draftExclusionStrs = new Set(draftExclusions.map(e => `${e.startingAddress}-${e.endingAddress}`));
        ...
        const baselineRouteStrs = new Set(baselineRoutes.map(r => `${r.destination}/${r.subnetMask}->${r.router}`));
        const draftRouteStrs = new Set(draftRoutes.map(r => `${r.destination}/${r.subnetMask}->${r.router}`));
        ...
        // Options changes
        if (draftScope.allowOnlyReservedLeases !== baselineScope.allowOnlyReservedLeases) {
            changes.push({
                type: 'modified',
                category: 'Options',
                description: draftScope.allowOnlyReservedLeases
        ...
        if (draftScope.dnsUpdates !== baselineScope.dnsUpdates) {
            changes.push({
                type: 'modified',
                category: 'Options',
                description: draftScope.dnsUpdates ? 'Enabled DNS updates' : 'Disabled DNS updates',
            });
        }
    ...
    const handleStaticRouteChange = (
        ...
        setDraftStaticRoutes((previous) =>
            previous.map((route) => (route.id === id ? { ...route, [key]: value } : route)),
        );
    };

    const handleRemoveStaticRoute = (id: string) => {
        setDraftStaticRoutes((previous) => previous.filter((route) => route.id !== id));
    };
    ...
    const handleVendorInfoChange = (id: string, key: 'identifier' | 'information', value: string) => {
        setDraftVendorInfo((previous) =>
            previous.map((entry) => (entry.id === id ? { ...entry, [key]: value } : entry)),
        );
    };

    const handleRemoveVendorInfo = (id: string) => {
        setDraftVendorInfo((previous) => previous.filter((entry) => entry.id !== id));
    };

    const handleAddGenericOption = () => {
        setDraftGenericOptions((previous) => [...previous, buildGenericOptionDraft()]);
    };

    const handleGenericOptionChange = (id: string, key: 'code' | 'value', value: string) => {
        ...
        setDraftGenericOptions((previous) =>
            previous.map((entry) => {
                ...
                if (mode === 'ascii') {
                    ...
                    if (!normalized) {
                        pushToast({
                            message: 'Provide a valid HEX sequence before switching to ASCII view.',
                            tone: 'info',
                            timeout: 5000,
                        });
                        return entry;
                    ...
                    if (ascii === undefined || !isPrintableAscii(ascii)) {
                        pushToast({
                            message: 'HEX value contains non-printable bytes; keeping HEX format.',
                            tone: 'info',
                            timeout: 5000,
                        });
                        return entry;
    ...
    const handleRemoveGenericOption = (id: string) => {
        setDraftGenericOptions((previous) => previous.filter((entry) => entry.id !== id));
    };
    ...
    const handleRemoveExclusion = (id: string) => {
        setDraftExclusions((previous) => previous.filter((entry) => entry.id !== id));
    };

    const handleAddReservedLease = () => {
        setDraftReservedLeases((previous) => [...previous, buildReservedLeaseDraft()]);
    };
    ...
    const handleRemoveReservedLease = (id: string) => {
        setDraftReservedLeases((previous) => previous.filter((entry) => entry.id !== id));
    };
                ...
                bulkSyncTargetNodeIds.map(async (nodeId) => {
                    try {
                        ...
                        if (!cancelled) {
                            ...
                            console.warn(`Failed to load scopes for target node ${nodeId}`, error);
                        }
                    }
                })
            );
    ...
    useEffect(() => {
        ...
        loadDhcpScopes(selectedNodeId)
            ...
            .catch((error: unknown) => {
                ...
                setScopeListError(error instanceof Error ? error.message : 'Failed to load DHCP scopes.');
            });
    ...
    useEffect(() => {
        ...
        loadDhcpScope(selectedNodeId, selectedScopeName)
            ...
            .catch((error: unknown) => {
                ...
                setScopeDetailError(error instanceof Error ? error.message : 'Failed to load scope details.');
                syncDraftWithScope(undefined);
            });
    ...
    const cloneSubmitDisabled =
        cloneState === 'loading' ||
        updateState === 'loading' ||
        trimmedStartingAddress.length === 0 ||
        trimmedEndingAddress.length === 0 ||
        trimmedSubnetMask.length === 0 ||
    ...
    const prepareScopeOverrides = (
        ...
        if (!trimmedStartingAddress || !trimmedEndingAddress || !trimmedSubnetMask) {
            onError('Starting address, ending address, and subnet mask are required.');
            return null;
        }
        ...
        if (vendorInfoSanitized.hasPartial) {
            onError('Vendor information entries require both identifier and information.');
            return null;
        }
        ...
        const normalizedServerHostName = normalizeOptionalString(draftServerHostName);
        const normalizedBootFileName = normalizeOptionalString(draftBootFileName);

        const currentStaticRoutesNormalized = (currentScope.staticRoutes ?? []).map((route) => ({
            destination: route.destination.trim(),
            subnetMask: route.subnetMask.trim(),
            router: route.router.trim(),
        }));

        const currentVendorInfoNormalized = (currentScope.vendorInfo ?? []).map((entry) => ({
            identifier: entry.identifier.trim(),
            information: entry.information.trim(),
        }));

        const currentGenericOptionsNormalized = (currentScope.genericOptions ?? []).map((option) => ({
            code: option.code,
            value: normalizeHexValue(option.value?.trim() ?? '') ?? option.value.trim(),
        }));

        const currentExclusionsNormalized = (currentScope.exclusions ?? []).map((entry) => ({
            startingAddress: entry.startingAddress.trim(),
            endingAddress: entry.endingAddress.trim(),
        }));

        const currentReservedNormalized = (currentScope.reservedLeases ?? []).map((entry) => ({
        ...
        if (trimmedStartingAddress !== currentScope.startingAddress) {
            overrides.startingAddress = trimmedStartingAddress;
        }

        if (trimmedEndingAddress !== currentScope.endingAddress) {
            overrides.endingAddress = trimmedEndingAddress;
        }

        if (trimmedSubnetMask !== currentScope.subnetMask) {
            overrides.subnetMask = trimmedSubnetMask;
        }

        const normalizedRouter = normalizeOptionalString(draftRouterAddress);
        if (normalizedRouter !== (currentScope.routerAddress ?? null)) {
            overrides.routerAddress = normalizedRouter;
        }

        const dnsList = splitListInput(draftDnsServers);
        if (!arraysEqual(dnsList, currentScope.dnsServers)) {
            overrides.dnsServers = dnsList;
        }

        const normalizedDomain = normalizeOptionalString(draftDomainName);
        const currentDomain = currentScope.domainName ?? '';
        if ((normalizedDomain ?? '') !== currentDomain) {
            overrides.domainName = normalizedDomain ?? '';
        }

        const searchList = splitListInput(draftDomainSearchList);
        if (!arraysEqual(searchList, currentScope.domainSearchList)) {
            overrides.domainSearchList = searchList;
        }

        if (leaseDays !== undefined && leaseDays !== (currentScope.leaseTimeDays ?? undefined)) {
            overrides.leaseTimeDays = leaseDays;
        }

        if (leaseHours !== undefined && leaseHours !== (currentScope.leaseTimeHours ?? undefined)) {
            overrides.leaseTimeHours = leaseHours;
        }

        if (leaseMinutes !== undefined && leaseMinutes !== (currentScope.leaseTimeMinutes ?? undefined)) {
            overrides.leaseTimeMinutes = leaseMinutes;
        }

        if (draftAllowOnlyReserved !== (currentScope.allowOnlyReservedLeases ?? false)) {
            overrides.allowOnlyReservedLeases = draftAllowOnlyReserved;
        }

        if (offerDelay !== undefined && offerDelay !== (currentScope.offerDelayTime ?? undefined)) {
        ...
        if (pingTimeout !== undefined && pingTimeout !== (currentScope.pingCheckTimeout ?? undefined)) {
            overrides.pingCheckTimeout = pingTimeout;
        }

        if (pingRetries !== undefined && pingRetries !== (currentScope.pingCheckRetries ?? undefined)) {
        ...
        if (draftUseThisDnsServer !== (currentScope.useThisDnsServer ?? false)) {
            overrides.useThisDnsServer = draftUseThisDnsServer;
        }
        ...
        if (
            !arraysEqualBy(staticRoutesSanitized.values, currentStaticRoutesNormalized, (route) =>
        ...
        if (
            !arraysEqualBy(vendorInfoSanitized.values, currentVendorInfoNormalized, (entry) =>
                `${entry.identifier}|${entry.information}`,
            )
        ) {
        ...
        if (
            !arraysEqualBy(genericOptionsSanitized.values, currentGenericOptionsNormalized, (entry) =>
        ...
        if (
            !arraysEqualBy(exclusionsSanitized.values, currentExclusionsNormalized, (entry) =>
                `${entry.startingAddress}|${entry.endingAddress}`,
            )
        ) {
        ...
        if (
            !arraysEqualBy(reservedLeasesSanitized.values, currentReservedNormalized, (entry) =>
        ...
        if (draftBlockLocallyAdministered !== (currentScope.blockLocallyAdministeredMacAddresses ?? false)) {
            overrides.blockLocallyAdministeredMacAddresses = draftBlockLocallyAdministered;
        }

        if (draftIgnoreClientIdentifier !== (currentScope.ignoreClientIdentifierOption ?? false)) {
    ...
    const handleClone = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!trimmedStartingAddress || !trimmedEndingAddress || !trimmedSubnetMask) {
            failClone('Starting address, ending address, and subnet mask are required.');
            return;
        }
        ...
        if (cloneMode === 'local') {
            ...
            const duplicate = scopes.some(
                (scope) => scope.name.toLowerCase() === trimmedNewScopeName.toLowerCase(),
            );

            if (duplicate) {
                failClone('A scope with that name already exists on this node. Choose another name.');
                return;
            }
        }

        const overrides = prepareScopeOverrides(failClone);
        if (overrides === null) {
            return;
        }
        ...
        try {
            const result: TechnitiumCloneDhcpScopeResult = await cloneDhcpScope(
                ...
                selectedScopeName,
                {
                    ...
                    newScopeName: trimmedNewScopeName.length > 0 ? trimmedNewScopeName : undefined,
                    enableOnTarget,
            ...
            if (result.targetNodeId === selectedNodeId) {
                try {
                    ...
                    console.warn('Failed to refresh DHCP scopes after local clone', refreshError);
                }
            }
        } catch (error) {
            failClone(error instanceof Error ? error.message : 'Failed to clone DHCP scope.');
            return;
        }
    };
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            setDetailCache((previous) => {
                const next = new Map(previous);
                next.set(buildScopeKey(selectedNodeId, response.data.scope.name), response.data.scope);
                return next;
            });
        } catch (error) {
            const message = error instanceof Error ? error.message : 'Failed to update DHCP scope.';
            setUpdateState('error');
            setUpdateError(message);
            setUpdateMessage(undefined);
            pushToast({ message, tone: 'error', timeout: 6000 });
    ...
    const getScopeExistsOnTargets = (scopeName: string): { exists: boolean; nodeIds: string[] } => {
    ...
    const handleBulkSyncStart = async () => {
        ...
        try {
            ...
            const message = error instanceof Error ? error.message : 'Failed to sync DHCP scopes.';
            pushToast({ message, tone: 'error', timeout: 6000 });
        } finally {
    ...
    const handleBulkSyncConfirm = async (request: DhcpBulkSyncRequest) => {
        ...
        try {
            ...
            for (const nodeResult of result.nodeResults) {
                if (nodeResult.syncedCount > 0) {
                    try {
                        ...
                        console.warn(`Failed to refresh scopes for node ${nodeResult.targetNodeId}`, refreshError);
                    }
                }
            }
            ...
            const message = error instanceof Error ? error.message : 'Failed to execute bulk sync.';
            pushToast({ message, tone: 'error', timeout: 6000 });
        } finally {
            setBulkSyncInProgress(false);
        }
            ...
            <section ref={pullToRefresh.containerRef} className="configuration">
                <header className="configuration__header">
                    <div>
                        ...
                        <p>
                            Manage DHCP scopes across nodes and sync configurations efficiently.
                        </p>
                    </div>
                </header>
                    ...
                    <>
                        ...
                        <section className="configuration-editor configuration-editor--stacked">
                            <div className="dhcp-page__panels">
                                <section className="dhcp-page__card">
                                    <header className="dhcp-page__card-header">
                                        <div>
                                            ...
                                            <p>Select a scope to review details and prepare a clone operation.</p>
                                        </div>
                                    </header>

                                    {scopeListState === 'loading' && (
                                        <div className="dhcp-page__placeholder">Loading DHCP scopes…</div>
                                    )}

                                    {scopeListState === 'error' && (
                                        <div className="dhcp-page__error">{scopeListError ?? 'Unable to load DHCP scopes.'}</div>
                                    )}
                                    ...
                                    {scopeListState === 'success' && scopes.length > 0 && (
                                        <div className="dhcp-page__table-wrapper" role="region" aria-live="polite">
                                                ...
                                                <tbody>
                                                    {scopes.map((scope) => {
                                                        ...
                                                        return (
                                                            <tr
                                                                key={scope.name}
                                                                className={isSelected ? 'dhcp-page__row dhcp-page__row--selected' : 'dhcp-page__row'}
                                                                onClick={() => handleScopeSelect(scope.name)}
                                                            >
                                                                <td data-title="Scope">
                                                                    <div className="dhcp-page__scope-name">{scope.name}</div>
                                ...
                                <section className="dhcp-page__card">
                                    {!selectedScopeName && scopes.length === 0 && scopeListState === 'success' && (
                                        <div className="dhcp-page__placeholder">
                                            No DHCP scopes configured on {selectedNodeLabel}. Create scopes in the Technitium DNS web interface.
                                        </div>
                                    )}

                                    {!selectedScopeName && scopes.length > 0 && (
                                        <div className="dhcp-page__placeholder">
                                            Select a scope from the list to view configuration and clone options.
                                        </div>
                                    )}

                                    {selectedScopeName && scopeDetailState === 'loading' && (
                                        <div className="dhcp-page__placeholder">Loading scope details…</div>
                                    )}

                                    {selectedScopeName && scopeDetailState === 'error' && (
                                        <div className="dhcp-page__error">{scopeDetailError ?? 'Unable to load scope details.'}</div>
                                    )}

                                    {selectedScopeName && scopeDetailState === 'success' && currentScope && (
                                        <>
                                            {/* Tab Switcher */}
                                            <div className="configuration__tab-switcher">
                                                <button
                                                ...
                                                >
                                                    ...
                                                    {isDirty && activeTab === 'scope-details' && (
                                                        <span className="configuration__tab-badge">●</span>
                                                    )}
                                                </button>
                                                <button
                                                    type="button"
                                            ...
                                            <div className="dhcp-page__details">
                                                    ...
                                                    <>
                                                        ...
                                                        >
                                                            ...
                                                            <div className="field-group field-group--inline">
                                                                <label className="checkbox" htmlFor="dhcp-scope-enabled-toggle">
                                                                    <input
                                                                        id="dhcp-scope-enabled-toggle"
                                                                        name="scopeEnabled"
                                                                        type="checkbox"
                                                                        checked={draftScopeEnabled}
                                                                        onChange={(event) => setDraftScopeEnabled(event.target.checked)}
                                                            ...
                                                            <div className="dhcp-page__clone-grid">
                                                                <div className={`field-group${!isValidIPv4Address(draftStartingAddress) ? ' field-group--error' : ''}`}>
                                                                    <label htmlFor="dhcp-starting-address">Starting address</label>
                                                                    <input
                                                                        id="dhcp-starting-address"
                                                                        name="startingAddress"
                                                                        type="text"
                                                                        value={draftStartingAddress}
                                                                        onChange={(event) => setDraftStartingAddress(event.target.value)}
                                                                    />
                                                                    {!isValidIPv4Address(draftStartingAddress) && (
                                                                        <span className="field-group__error-message">
                                                                            Invalid IPv4 address (each octet must be 0-255)
                                                                        </span>
                                                                    )}
                                                                </div>
                                                                <div className={`field-group${!isValidIPv4Address(draftEndingAddress) ? ' field-group--error' : ''}`}>
                                                                    <label htmlFor="dhcp-ending-address">Ending address</label>
                                                                    <input
                                                                        id="dhcp-ending-address"
                                                                        name="endingAddress"
                                                                        type="text"
                                                                        value={draftEndingAddress}
                                                                        onChange={(event) => setDraftEndingAddress(event.target.value)}
                                                                    />
                                                                    {!isValidIPv4Address(draftEndingAddress) && (
                                                                        <span className="field-group__error-message">
                                                                            Invalid IPv4 address (each octet must be 0-255)
                                                                        </span>
                                                                    )}
                                                                </div>
                                                                <div className={`field-group${!isValidIPv4Address(draftSubnetMask) ? ' field-group--error' : ''}`}>
                                                                    <label htmlFor="dhcp-subnet-mask">Subnet mask</label>
                                                                    <input
                                                                        id="dhcp-subnet-mask"
                                                                        name="subnetMask"
                                                                        type="text"
                                                                        value={draftSubnetMask}
                                                                        onChange={(event) => setDraftSubnetMask(event.target.value)}
                                                                    />
                                                                ...
                                                                <div className={`field-group${!isValidIPv4Address(draftRouterAddress) ? ' field-group--error' : ''}`}>
                                                                    <label htmlFor="dhcp-router-address">Router address</label>
                                                                    <input
                                                                        ...
                                                                        onChange={(event) => setDraftRouterAddress(event.target.value)}
                                                                    />
                                                                    {!isValidIPv4Address(draftRouterAddress) && (
                                                                        <span className="field-group__error-message">
                                                                            Invalid IPv4 address (each octet must be 0-255)
                                                                        </span>
                                                                    )}
                                                            ...
                                                            <div className="dhcp-page__clone-grid">
                                                                <div className="field-group">
                                                                    ...
                                                                    <input
                                                                        ...
                                                                        onChange={(event) => setDraftLeaseDays(event.target.value)}
                                                                    />
                                                                </div>
                                                                <div className="field-group">
                                                                    <label htmlFor="dhcp-lease-hours">Lease duration (hours)</label>
                                                                    <input
                                                                        id="dhcp-lease-hours"
                                                                        name="leaseHours"
                                                                        type="number"
                                                                        ...
                                                                        onChange={(event) => setDraftLeaseHours(event.target.value)}
                                                                    />
                                                                </div>
                                                                <div className="field-group">
                                                                    <label htmlFor="dhcp-lease-minutes">Lease duration (minutes)</label>
                                                                    <input
                                                                        ...
                                                                        onChange={(event) => setDraftLeaseMinutes(event.target.value)}
                                                                    />
                                                                </div>
                                                            </div>
                                                            ...
                                                            <details className="dhcp-page__advanced">
                                                                <summary>Lease pacing &amp; validation</summary>
                                                                <div className="dhcp-page__clone-grid dhcp-page__advanced-grid">
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-offer-delay">Offer delay (ms)</label>
                                                                        <input
                                                                            id="dhcp-offer-delay"
                                                                            name="offerDelay"
                                                                            type="number"
                                                                            ...
                                                                            onChange={(event) => setDraftOfferDelay(event.target.value)}
                                                                        />
                                                                        <p className="field-hint">Delay DHCP offers to avoid collisions.</p>
                                                                    </div>
                                                                    <div className="field-group field-group--inline">
                                                                        <label className="checkbox" htmlFor="dhcp-ping-check">
                                                                            <input
                                                                                id="dhcp-ping-check"
                                                                                name="pingCheckEnabled"
                                                                                type="checkbox"
                                                                                checked={draftPingCheckEnabled}
                                                                                onChange={(event) => setDraftPingCheckEnabled(event.target.checked)}
                                                                            />
                                                                            <span>Ping address before offering lease</span>
                                                                        </label>
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-ping-timeout">Ping timeout (ms)</label>
                                                                        <input
                                                                            ...
                                                                            onChange={(event) => setDraftPingTimeout(event.target.value)}
                                                                        />
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-ping-retries">Ping retries</label>
                                                                        <input
                                                                            ...
                                                                            onChange={(event) => setDraftPingRetries(event.target.value)}
                                                                        />
                                                                    </div>
                                                                </div>
                                                            </details>
                                                            ...
                                                            <details className="dhcp-page__advanced">
                                                                <summary>DNS updates &amp; server identity</summary>
                                                                <div className="dhcp-page__clone-grid dhcp-page__advanced-grid">
                                                                    <div className="field-group field-group--inline">
                                                                        <label className="checkbox" htmlFor="dhcp-dns-updates">
                                                                            <input
                                                                                id="dhcp-dns-updates"
                                                                                name="dnsUpdates"
                                                                                type="checkbox"
                                                                                checked={draftDnsUpdates}
                                                                                onChange={(event) => setDraftDnsUpdates(event.target.checked)}
                                                                            />
                                                                            <span>Allow dynamic DNS updates</span>
                                                                        </label>
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-dns-ttl">DNS TTL (seconds)</label>
                                                                        <input
                                                                            id="dhcp-dns-ttl"
                                                                            name="dnsTtl"
                                                                            type="number"
                                                                            min="0"
                                                                            value={draftDnsTtl}
                                                                            onChange={(event) => setDraftDnsTtl(event.target.value)}
                                                                        />
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-server-address">Server address override</label>
                                                                        <input
                                                                            ...
                                                                            placeholder="Optional"
                                                                            onChange={(event) => setDraftServerAddress(event.target.value)}
                                                                        />
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-server-host">Server hostname</label>
                                                                        <input
                                                                            id="dhcp-server-host"
                                                                            name="serverHostName"
                                                                            type="text"
                                                                            ...
                                                                            placeholder="Optional"
                                                                            onChange={(event) => setDraftServerHostName(event.target.value)}
                                                                        />
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-boot-file">Boot file name</label>
                                                                        <input
                                                                            id="dhcp-boot-file"
                                                                            name="bootFileName"
                                                                            type="text"
                                                                            ...
                                                                            placeholder="Optional"
                                                                            onChange={(event) => setDraftBootFileName(event.target.value)}
                                                                        />
                                                                    </div>
                                                                    <div className="field-group field-group--inline">
                                                                        <label className="checkbox" htmlFor="dhcp-use-this-dns-advanced">
                                                                            <input
                                                                                ...
                                                                                onChange={(event) => setDraftUseThisDnsServer(event.target.checked)}
                                                                            />
                                                                            <span>Advertise this server as DNS</span>
                                                                        </label>
                                                                    </div>
                                                                </div>
                                                            </details>
                                                            ...
                                                            <details className="dhcp-page__advanced">
                                                                ...
                                                                <div className="dhcp-page__clone-grid dhcp-page__advanced-grid">
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-wins-servers">WINS servers (one per line)</label>
                                                                        <textarea
                                                                            id="dhcp-wins-servers"
                                                                            name="winsServers"
                                                                            value={draftWinsServers}
                                                                            onChange={(event) => setDraftWinsServers(event.target.value)}
                                                                        />
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-ntp-servers">NTP servers (one per line)</label>
                                                                        <textarea
                                                                            id="dhcp-ntp-servers"
                                                                            name="ntpServers"
                                                                            value={draftNtpServers}
                                                                            onChange={(event) => setDraftNtpServers(event.target.value)}
                                                                        />
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-ntp-domains">NTP domains (one per line)</label>
                                                                        <textarea
                                                                            id="dhcp-ntp-domains"
                                                                            name="ntpDomains"
                                                                            value={draftNtpDomains}
                                                                            onChange={(event) => setDraftNtpDomains(event.target.value)}
                                                                        />
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-capwap">CAPWAP controllers (one per line)</label>
                                                                        <textarea
                                                                            id="dhcp-capwap"
                                                                            name="capwapControllers"
                                                                            value={draftCapwapControllers}
                                                                            onChange={(event) => setDraftCapwapControllers(event.target.value)}
                                                                        />
                                                                    </div>
                                                                    <div className="field-group">
                                                                        <label htmlFor="dhcp-tftp">TFTP servers (one per line)</label>
                                                                        <textarea
                                                                            id="dhcp-tftp"
                                                                            name="tftpServers"
                                                                            value={draftTftpServers}
                                                                            onChange={(event) => setDraftTftpServers(event.target.value)}
                                                                        />
                                                                    </div>
                                                                </div>
                                                            </details>
                                                            ...
                                                            <details className="dhcp-page__advanced">
                                                                ...
                                                                <div className="dhcp-page__list-section">
                                                                    <header>
                                                                        <h4>Static routes</h4>
                                                                        <p>Define classless static routes advertised via option 121.</p>
                                                                    </header>
                                                                    {draftStaticRoutes.length === 0 && (
                                                                        <p className="dhcp-page__list-empty">No static routes defined.</p>
                                                                    )}
                                                                    {draftStaticRoutes.map((route) => (
                                                                        <div key={route.id} className="dhcp-page__list-row">
                                                                            <div className="dhcp-page__list-row-grid">
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-static-destination-${route.id}`}>Destination</label>
                                                                                    <input
                                                                                        ...
                                                                                        onChange={(event) =>
                                                                                            handleStaticRouteChange(route.id, 'destination', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-static-mask-${route.id}`}>Subnet mask</label>
                                                                                    <input
                                                                                        ...
                                                                                        onChange={(event) =>
                                                                                            handleStaticRouteChange(route.id, 'subnetMask', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-static-router-${route.id}`}>Router</label>
                                                                                    <input
                                                                                        ...
                                                                                        value={route.router}
                                                                                        onChange={(event) =>
                                                                                            handleStaticRouteChange(route.id, 'router', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                            </div>
                                                                            <button
                                                                                ...
                                                                                onClick={() => handleRemoveStaticRoute(route.id)}
                                                                            >
                                                                                Remove
                                                                            </button>
                                                                        </div>
                                                                    ))}
                                                                    <button type="button" className="secondary" onClick={handleAddStaticRoute}>
                                                                        Add static route
                                                                    </button>
                                                                </div>
                                                                ...
                                                                <div className="dhcp-page__list-section">
                                                                    <header>
                                                                        ...
                                                                        <p>Supply vendor-specific identifier and payload.</p>
                                                                    </header>
                                                                    {draftVendorInfo.length === 0 && (
                                                                        <p className="dhcp-page__list-empty">No vendor entries defined.</p>
                                                                    )}
                                                                    {draftVendorInfo.map((entry) => (
                                                                        <div key={entry.id} className="dhcp-page__list-row">
                                                                            <div className="dhcp-page__list-row-grid">
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-vendor-identifier-${entry.id}`}>Identifier</label>
                                                                                    <input
                                                                                        ...
                                                                                        onChange={(event) =>
                                                                                            handleVendorInfoChange(entry.id, 'identifier', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-vendor-info-${entry.id}`}>Information</label>
                                                                                    <textarea
                                                                                        id={`dhcp-vendor-info-${entry.id}`}
                                                                                        name={`vendorInformation-${entry.id}`}
                                                                                        value={entry.information}
                                                                                        onChange={(event) =>
                                                                                            handleVendorInfoChange(entry.id, 'information', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                            </div>
                                                                            <button
                                                                                type="button"
                                                                                className="secondary dhcp-page__list-remove"
                                                                                onClick={() => handleRemoveVendorInfo(entry.id)}
                                                                            >
                                                                                Remove
                                                                            </button>
                                                                        </div>
                                                                    ))}
                                                                    <button type="button" className="secondary" onClick={handleAddVendorInfo}>
                                                                        Add vendor entry
                                                                    </button>
                                                                </div>
                                                                ...
                                                                <div className="dhcp-page__list-section">
                                                                    <header>
                                                                        ...
                                                                        <p>Specify additional DHCP options by code.</p>
                                                                    </header>
                                                                    {draftGenericOptions.length === 0 && (
                                                                        <p className="dhcp-page__list-empty">No generic options defined.</p>
                                                                    )}
                                                                    {draftGenericOptions.map((entry) => {
                                                                        const conversion = describeGenericOptionValue(entry);
                                                                        ...
                                                                        const asciiToggleDisabled =
                                                                            ...
                                                                            (conversion.status === 'invalid' ||
                                                                                (conversion.status === 'hex' && !conversion.ascii));

                                                                        return (
                                                                            <div key={entry.id} className="dhcp-page__list-row">
                                                                                <div className="dhcp-page__list-row-grid dhcp-page__list-row-grid--option">
                                                                                    <div className="field-group">
                                                                                        <label htmlFor={codeId}>Option code</label>
                                                                                        <input
                                                                                            id={codeId}
                                                                                            ...
                                                                                            value={entry.code}
                                                                                            onChange={(event) =>
                                                                                                handleGenericOptionChange(entry.id, 'code', event.target.value)
                                                                                            }
                                                                                        />
                                                                                    </div>
                                                                                    <div className="field-group">
                                                                                        <div className="dhcp-page__value-header">
                                                                                            ...
                                                                                            <div className="dhcp-page__value-mode">
                                                                                                ...
                                                                                                <div className="dhcp-page__value-mode-buttons">
                                                                                                    <button
                                                                                                        type="button"
                                                                                                        className={entry.mode === 'ascii'
                                                                                                            ? 'active'
                                                                                                            : ''}
                                                                                                        onClick={() => handleGenericOptionModeChange(entry.id, 'ascii')}
                                                                                                        disabled={asciiToggleDisabled}
                                                                                                    >
                                                                                                        ASCII
                                                                                                    </button>
                                                                                                    <button
                                                                                                        type="button"
                                                                                                        className={entry.mode === 'hex'
                                                                                                            ? 'active'
                                                                                                            : ''}
                                                                                                        onClick={() => handleGenericOptionModeChange(entry.id, 'hex')}
                                                                                                    >
                                                                                                        HEX
                                                                                                    </button>
                                                                                                </div>
                                                                                        ...
                                                                                        <textarea
                                                                                            ...
                                                                                            value={entry.value}
                                                                                            onChange={(event) =>
                                                                                                handleGenericOptionChange(entry.id, 'value', event.target.value)
                                                                                            }
                                                                                            placeholder={
                                                                                                entry.mode === 'ascii'
                                                                                            ...
                                                                                            spellCheck={entry.mode === 'ascii'}
                                                                                            rows={entry.mode === 'ascii' ? 2 : 3}
                                                                                            className="dhcp-page__textarea"
                                                                                        />
                                                                                        {conversion.status === 'ascii' && (
                                                                                            <p className="dhcp-page__conversion">
                                                                                                <strong>HEX:</strong> {conversion.hex}
                                                                                            </p>
                                                                                        )}
                                                                                        {conversion.status === 'hex' && (
                                                                                            <p className="dhcp-page__conversion">
                                                                                                <strong>Normalized HEX:</strong> {conversion.hex}
                                                                                                {conversion.ascii && (
                                                                                                    <>
                                                                                                        <br />
                                                                                                        <strong>ASCII:</strong> {conversion.ascii}
                                                                                                    </>
                                                                                                )}
                                                                                                {conversion.hasNonPrintable && (
                                                                                                    <>
                                                                                                        <br />
                                                                                                        <em>Contains non-printable characters</em>
                                                                                                    </>
                                                                                                )}
                                                                                            </p>
                                                                                        )}
                                                                                        {conversion.status === 'invalid' && (
                                                                                            <p className="dhcp-page__conversion dhcp-page__conversion--error">
                                                                                                Enter HEX bytes like 01:02:FF or switch to ASCII input.
                                                                                            </p>
                                                                                        )}
                                                                                    </div>
                                                                                </div>
                                                                                <button
                                                                                    type="button"
                                                                                    className="secondary dhcp-page__list-remove"
                                                                                    onClick={() => handleRemoveGenericOption(entry.id)}
                                                                                >
                                                                                    Remove
                                                                                </button>
                                                                            </div>
                                                                    ...
                                                                    <button type="button" className="secondary" onClick={handleAddGenericOption}>
                                                                        Add option
                                                                    </button>
                                                                </div>
                                                            </details>
                                                            ...
                                                            <details className="dhcp-page__advanced">
                                                                <summary>Exclusions &amp; reservations</summary>
                                                                <div className="dhcp-page__list-section">
                                                                    <header>
                                                                        <h4>Exclusion ranges</h4>
                                                                        <p>Reserve blocks of addresses that should not be handed out.</p>
                                                                    </header>
                                                                    {draftExclusions.length === 0 && (
                                                                        <p className="dhcp-page__list-empty">No exclusion ranges defined.</p>
                                                                    )}
                                                                    {draftExclusions.map((entry) => (
                                                                        <div key={entry.id} className="dhcp-page__list-row">
                                                                            <div className="dhcp-page__list-row-grid">
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-exclusion-start-${entry.id}`}>Start address</label>
                                                                                    <input
                                                                                        ...
                                                                                        onChange={(event) =>
                                                                                            handleExclusionChange(entry.id, 'startingAddress', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-exclusion-end-${entry.id}`}>End address</label>
                                                                                    <input
                                                                                        ...
                                                                                        onChange={(event) =>
                                                                                            handleExclusionChange(entry.id, 'endingAddress', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                            </div>
                                                                            <button
                                                                                type="button"
                                                                                className="secondary dhcp-page__list-remove"
                                                                                onClick={() => handleRemoveExclusion(entry.id)}
                                                                            >
                                                                                Remove
                                                                            </button>
                                                                        </div>
                                                                    ))}
                                                                    <button type="button" className="secondary" onClick={handleAddExclusion}>
                                                                        Add exclusion
                                                                    </button>
                                                                </div>
                                                                ...
                                                                <div className="dhcp-page__list-section">
                                                                    <header>
                                                                        ...
                                                                        <p>Pin specific MAC addresses to fixed IP assignments.</p>
                                                                    </header>
                                                                    {draftReservedLeases.length === 0 && (
                                                                        <p className="dhcp-page__list-empty">No reserved leases defined.</p>
                                                                    )}
                                                                    {draftReservedLeases.map((entry) => (
                                                                        <div key={entry.id} className="dhcp-page__list-row">
                                                                            <div className="dhcp-page__list-row-grid dhcp-page__list-row-grid--wide">
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-reserved-host-${entry.id}`}>Hostname</label>
                                                                                    <input
                                                                                        ...
                                                                                        placeholder="Optional"
                                                                                        onChange={(event) =>
                                                                                            handleReservedLeaseChange(entry.id, 'hostName', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-reserved-mac-${entry.id}`}>Hardware address</label>
                                                                                    <input
                                                                                        ...
                                                                                        onChange={(event) =>
                                                                                            handleReservedLeaseChange(entry.id, 'hardwareAddress', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-reserved-ip-${entry.id}`}>IP address</label>
                                                                                    <input
                                                                                        ...
                                                                                        value={entry.address}
                                                                                        onChange={(event) =>
                                                                                            handleReservedLeaseChange(entry.id, 'address', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                            </div>
                                                                            <div className="dhcp-page__list-row-grid dhcp-page__list-row-grid--wide">
                                                                                <div className="field-group">
                                                                                    <label htmlFor={`dhcp-reserved-comment-${entry.id}`}>Comment</label>
                                                                                    <textarea
                                                                                        ...
                                                                                        placeholder="Optional"
                                                                                        onChange={(event) =>
                                                                                            handleReservedLeaseChange(entry.id, 'comments', event.target.value)
                                                                                        }
                                                                                    />
                                                                                </div>
                                                                            </div>
                                                                            <button
                                                                                type="button"
                                                                                className="secondary dhcp-page__list-remove"
                                                                                onClick={() => handleRemoveReservedLease(entry.id)}
                                                                            >
                                                                                Remove
                                                                            </button>
                                                                        </div>
                                                                    ))}
                                                                    <button type="button" className="secondary" onClick={handleAddReservedLease}>
                                                                        Add reserved lease
                                                                    </button>
                                                                </div>
                                                                ...
                                                                <div className="dhcp-page__advanced-grid dhcp-page__advanced-grid--toggles">
                                                                    <div className="field-group field-group--inline">
                                                                        <label className="checkbox" htmlFor="dhcp-block-locally-admin">
                                                                            <input
                                                                                id="dhcp-block-locally-admin"
                                                                                name="blockLocallyAdministered"
                                                                                type="checkbox"
                                                                                checked={draftBlockLocallyAdministered}
                                                                                onChange={(event) =>
                                                                                    setDraftBlockLocallyAdministered(event.target.checked)
                                                                                }
                                                                            />
                                                                            <span>Block locally administered MAC addresses</span>
                                                                        </label>
                                                                    </div>
                                                                    <div className="field-group field-group--inline">
                                                                        <label className="checkbox" htmlFor="dhcp-ignore-client-id">
                                                                            <input
                                                                                ...
                                                                                onChange={(event) =>
                                                                                    setDraftIgnoreClientIdentifier(event.target.checked)
                                                                                }
                                                                            />
                                                                            <span>Ignore client identifier option</span>
                                                                        </label>
                                                                    </div>
                                                                </div>
                                                            </details>
                                                                ...
                                                                <footer className="configuration-editor__footer configuration-editor__footer--sticky">
                                                                    ...
                                                                    <div className="configuration-editor__footer-actions">
                                                                        <button
                                                                            ...
                                                                            disabled={updateState === 'loading' || !selectedScopeName}
                                                                    ...
                                                                    {updateState === 'success' && updateMessage && (
                                                ...
                                                {activeTab === 'clone' && (
                                                    ...
                                                    >
                                                        <fieldset>
                                                            <legend id="dhcp-clone-form-title">Clone scope</legend>
                                                            <p className="dhcp-page__clone-intro">
                                                                {cloneMode === 'local'
                                                            ...
                                                            <div className="dhcp-page__clone-mode" role="group" aria-label="Clone destination">
                                                                <label
                                                                    className={`dhcp-page__clone-mode-option${cloneMode === 'local' ? ' dhcp-page__clone-mode-option--active' : ''
                                                                        }`}
                                                                >
                                                                    <input
                                                                        type="radio"
                                                                    ...
                                                                    />
                                                                    <div>
                                                                        <strong><FontAwesomeIcon icon={faClipboard} /> Duplicate on this node</strong>
                                                                        <span>Create a copy with a new name on {selectedNodeId || 'the same server'} (useful for similar subnets)</span>
                                                                    </div>
                                                                </label>
                                                                <label
                                                                    className={`dhcp-page__clone-mode-option${cloneMode === 'remote' ? ' dhcp-page__clone-mode-option--active' : ''
                                                            ...
                                                            {cloneMode === 'remote' && (
                                                                <div className="field-group">
                                                                    <label htmlFor="dhcp-target-node">Destination node</label>
                                                                    <select
                                                                        id="dhcp-target-node"
                                                                        name="targetNode"
                                                                        value={targetNodeId}
                                                            ...
                                                            <div className="field-group">
                                                                <label htmlFor="dhcp-new-name">
                                                                    New scope name {cloneMode === 'local' ? '(required)' : '(optional)'}
                                                                </label>
                                                                <input
                                                                    id="dhcp-new-name"
                                                                    name="newScopeName"
                                                                    type="text"
                                                                    value={newScopeName}
                                                                    placeholder={cloneMode === 'local' && selectedScopeName
                                                                ...
                                                                />
                                                                {cloneMode === 'local' && (
                                                                    <p className="field-hint">Name must be unique on this node.</p>
                                                                )}
                                                            </div>

                                                            <div className="field-group field-group--inline">
                                                                <label className="checkbox" htmlFor="dhcp-enable-target">
                                                                    <input
                                                                        id="dhcp-enable-target"
                                                                        name="enableOnTarget"
                                                                        type="checkbox"
                                                                        checked={enableOnTarget}
                                                                        onChange={(event) => setEnableOnTarget(event.target.checked)}
                                                                    />
                                                                    <span>
                                                                        {isLocalClone
                                                            ...
                                                            <div className="dhcp-page__clone-section-header">
                                                                ...
                                                                <p className="field-hint">
                                                                    The cloned scope will use the current scope configuration from the "Scope Configuration" section above.
                                                                    {cloneMode === 'remote' && ' Make any necessary adjustments in that section before cloning to the target node.'}
                                                                    {cloneMode === 'local' && ' Make any necessary adjustments in that section before creating the copy.'}
                                                                </p>
                                                            </div>
                                                            ...
                                                            <div className="dhcp-page__clone-actions">
                                                                ...
                                                                <button
                                                                    ...
                                                                    disabled={cloneState === 'loading' || updateState === 'loading'}
                                                                >
                                                                    Reset fields
                                                                </button>
                                                            </div>
                        ...
                        <div className="configuration-editor">
                            <div className="dhcp-bulk-sync-inline">
                                <div className="dhcp-bulk-sync-inline__header">
                                    ...
                                    <p>
                                        Synchronize DHCP scopes across multiple nodes efficiently. Configure your
                                        source node, target nodes, and sync strategy below.
                                    </p>
                                </div>
                                    ...
                                    <div className="dhcp-bulk-sync-inline__form">
                                                ...
                                                {nodes.map((node) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            type="button"
                                                            className={`node-selector__card ${isSelected ? 'node-selector__card--selected' : ''}`}
                                                            onClick={() => handleBulkSyncSourceChange(node.id)}
                                        ...
                                        <div className="field-group">
                                            ...
                                            {bulkSyncAvailableTargets.length === 0 ? (
                                                <div className="field-group__hint field-group__hint--warning" style={{ marginTop: '0.5rem' }}>
                                                ...
                                                <>
                                                    ...
                                                    <div className="node-selector__cards">
                                                        {bulkSyncAvailableTargets.map((node) => {
                                                            const isSelected = bulkSyncTargetNodeIds.includes(node.id);
                                                            const scopeCount = scopeCountByNode.get(node.id) ?? 0;
                                                            return (
                                                                <button
                                                                    key={node.id}
                                                                    type="button"
                                                                    className={`node-selector__card ${isSelected ? 'node-selector__card--selected' : ''}`}
                                                                    onClick={() => handleBulkSyncTargetToggle(node.id)}
                                                                    disabled={bulkSyncInProgress}
                                                                >
                                            ...
                                            <div className="dhcp-bulk-sync-modal__strategy-list">
                                                <label className="dhcp-bulk-sync-modal__strategy-item">
                                                    <input
                                                        ...
                                                        onChange={(e) => setBulkSyncStrategy(e.target.value as DhcpBulkSyncStrategy)}
                                                ...
                                                <label className="dhcp-bulk-sync-modal__strategy-item">
                                                    ...
                                                    <div className="dhcp-bulk-sync-modal__strategy-content">
                                                        ...
                                                        <div className="dhcp-bulk-sync-modal__strategy-description">
                                                            Delete all scopes on target nodes first, then copy all scopes from source. Makes targets identical to source.
                                                        </div>
                                                    </div>
                                                </label>
                                                ...
                                                <label className="dhcp-bulk-sync-modal__strategy-item">
                                                    ...
                                                    <div className="dhcp-bulk-sync-modal__strategy-content">
                                                        ...
                                                        <div className="dhcp-bulk-sync-modal__strategy-description">
                                                            Add missing scopes and update existing scopes to align with source. Preserves any extra scopes on targets.
                                                        </div>
                                                    </div>
                                                </label>
                                            </div>
                                        ...
                                        <div className="field-group">
                                            <label className="dhcp-bulk-sync-modal__option-item">
                                                <input
                                                    ...
                                                    onChange={(e) => setBulkSyncEnableOnTarget(e.target.checked)}
                                            ...
                                            <div className="dhcp-bulk-sync-inline__preview">
                                                <div className="dhcp-bulk-sync-inline__preview-header">
                                                    ...
                                                    <span className="dhcp-bulk-sync-inline__preview-subtitle">
                                                        Scopes to be copied from {nodes.find(n => n.id === bulkSyncSourceNodeId)?.name || bulkSyncSourceNodeId} → {bulkSyncTargetNodeIds.length} target node{bulkSyncTargetNodeIds.length !== 1 ? 's' : ''}
                                                    </span>
                                                </div>
                                                ...
                                                {bulkSyncSourceScopesLoading ? (
                                                    ...
                                                    <div className="dhcp-bulk-sync-inline__preview-empty">
                                                        ...
                                                        <p>Select a different source node that has DHCP scopes configured.</p>
                                                    </div>
                                                ) : (
                                                    <div className="dhcp-bulk-sync-inline__preview-list">
                                                        {bulkSyncSourceScopes.map((scope) => {
                                                            const { exists, nodeIds } = getScopeExistsOnTargets(scope.name);
                                                            const isExpanded = bulkSyncExpandedScopes.has(scope.name);
                                                            ...
                                                            return (
                                                                ...
                                                                >
                                                                    <button
                                                                        ...
                                                                        onClick={() => handleBulkSyncToggleScopeExpanded(scope.name)}
                                                                        disabled={bulkSyncInProgress}
                                                                    >
                                                                        ...
                                                                        <div className="dhcp-bulk-sync-inline__preview-content">
                                                                                ...
                                                                                <div className="dhcp-bulk-sync-inline__preview-note">
                                                                                    {targetsToSync.length > 0 ? (
                                                                                        <>
                                                                                            ...
                                                                                            <div>⊘ Already exists on: {targetsToSkip.map(id => nodes.find(n => n.id === id)?.name || id).join(', ')}</div>
                                                                                        </>
                                                                                    ) : (
                                                                            ...
                                                                            {bulkSyncStrategy === 'overwrite-all' && nodeIds.length > 0 && (
                                                                                <div className="dhcp-bulk-sync-inline__preview-note">
                                                                                    Will overwrite existing scope on: {nodeIds.map(id => nodes.find(n => n.id === id)?.name || id).join(', ')}
                                                                                </div>
                                                                            )}
                                                                        </div>
                                                                    </button>
                                        ...
                                        <div className="dhcp-bulk-sync-inline__actions">
                                            <button
                                                ...
                                                disabled={!bulkSyncCanStart || bulkSyncSourceScopes.length === 0}
                                                title={!bulkSyncCanStart ? 'Select at least one target node' : bulkSyncSourceScopes.length === 0 ? 'Source node has no scopes to sync' : 'Start bulk sync operation'}
                                            >
                ...
                <DhcpBulkSyncModal
                    ...
                    availableNodes={nodes.map((node) => ({ id: node.id, name: node.name }))}
