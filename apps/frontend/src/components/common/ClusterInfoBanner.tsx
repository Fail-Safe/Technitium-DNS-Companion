 ...
 */
export function ClusterInfoBanner({
  primaryNodeName,
  show,
}: ClusterInfoBannerProps) {
  ...
  // IMPORTANT: `show` can be false transiently while cluster state loads.
  // If we clear localStorage immediately when `show` is false, a reload/hot-reload
  // can cause the banner to re-appear even after the user dismissed it.
  // Track whether clustering has ever been shown (show=true) in this session.
  ...
  useEffect(() => {
    ...
    // Only clear dismissal if clustering was previously enabled in this session.
    // This avoids clearing during initial load when cluster status is still unknown.
    if (hasEverShownRef.current) {
    ...
    return (
      <div className="cluster-info-banner cluster-info-banner--loading">
        <div className="cluster-info-banner__content">
          <h3 className="cluster-info-banner__title">
          ...
          <div className="cluster-info-banner cluster-info-banner--dropdown">
            ...
            <div className="cluster-info-banner__content">
              ...
              <p className="cluster-info-banner__message">
                Your Technitium DNS servers are running in a cluster.
                Configuration changes can only be made on the{" "}
                <strong>Primary node ({primaryNodeName || "Unknown"})</strong>.
                Secondary nodes will automatically receive updates via zone
                transfers.
              </p>
            </div>
            <button
  ...
  return (
    <div className="cluster-info-banner">
      ...
      <div className="cluster-info-banner__content">
        ...
        <p className="cluster-info-banner__message">
          Your Technitium DNS servers are running in a cluster. Configuration
