...
export function ConfigurationSyncView({
  ...
  const targetNode = availableNodes.find(
    (n) =>
      n.nodeId === (viewMode === "primary-secondaries" ? "" : targetNodeId),
  );
    ...
    for (const groupName of allGroupNames) {
      ...
      if (!sourceGroup && targetGroup) {
        ...
        const isDifferent =
          ...
          !compareUrlArrays(
            sourceGroup.blockListUrls,
            targetGroup.blockListUrls,
          ) ||
          !compareUrlArrays(
            sourceGroup.allowListUrls,
            targetGroup.allowListUrls,
          ) ||
          !compareUrlArrays(
        ...
        if (isDifferent || hasSettingsDifferences) {
          ...
          detailedDiff = {
            blocked: calculateDomainDiff(
              sourceGroup.blocked,
              targetGroup.blocked,
            ),
            allowed: calculateDomainDiff(
              sourceGroup.allowed,
              targetGroup.allowed,
            ),
        ...
        diffs.push({
          name: groupName,
          status:
            isDifferent || hasSettingsDifferences ? "different" : "in-sync",
          sourceStats,
          targetStats,
          detailedDiff,
          settingsDifferences:
            hasSettingsDifferences ? settingsDifferences : undefined,
        });
      }
    }
  ...
  const configDifferences = useMemo(() => {
    ...
    if (
      sourceNode.config.blockingAnswerTtl !==
      targetNode.config.blockingAnswerTtl
    ) {
      globalDifferent.push({
    ...
    if (
      sourceNode.config.blockListUrlUpdateIntervalHours !==
      targetNode.config.blockListUrlUpdateIntervalHours
    ) {
      globalDifferent.push({
    ...
    if (
      sourceNode.config.blockListUrlUpdateIntervalMinutes !==
      targetNode.config.blockListUrlUpdateIntervalMinutes
    ) {
      globalDifferent.push({
        field: "blockListUrlUpdateIntervalMinutes",
        sourceValue: sourceNode.config.blockListUrlUpdateIntervalMinutes,
        targetValue: targetNode.config.blockListUrlUpdateIntervalMinutes,
      });
    }
