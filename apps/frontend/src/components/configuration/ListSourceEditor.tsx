...
export function ListSourceEditor({
  config,
  onSave,
  onDirtyChange,
  disabled,
}: ListSourceEditorProps) {
  // Generate unique IDs for form elements
  const refreshIntervalId = useId();
  const refreshIntervalMinutesId = useId();

  // Helper function to deep clone config
  const cloneConfig = useCallback(
    (cfg: AdvancedBlockingConfig): AdvancedBlockingConfig => {
  ...
  const [isRefreshMinutesFocused, setIsRefreshMinutesFocused] = useState(false);
  const [activeListType, setActiveListType] =
    useState<ListType>("blockListUrls");
  ...
  const pendingChanges = useMemo(() => {
    ...
    if (
      ...
      draftConfig.blockListUrlUpdateIntervalMinutes
    ) {
      changes.push({
        type: "modified",
        listType: "Global Settings",
        field: "List source refresh interval (minutes)",
        oldValue: config.blockListUrlUpdateIntervalMinutes ?? "not set",
        newValue: draftConfig.blockListUrlUpdateIntervalMinutes ?? "not set",
      });
    }
    ...
    draftConfig.groups.forEach((draftGroup) => {
      ...
      const originalGroup = config.groups.find(
        (g) => g.name === draftGroup.name,
      );

      LIST_TYPES.forEach((listType) => {
        const draftUrls = new Set(
          ((draftGroup[listType.key] as AdvancedBlockingUrlEntry[]) || []).map(
            getUrlString,
          ),
        );
        const originalUrls = new Set(
          originalGroup ?
            (
              (originalGroup[listType.key] as AdvancedBlockingUrlEntry[]) || []
            ).map(getUrlString)
          : [],
        );
  ...
  const getGroupUrls = useCallback(
    (groupName: string, listType: ListType): string[] => {
    ...
    async (url: string) => {
      ...
      updatedConfig.groups = updatedConfig.groups.map((group) => {
        ...
        return {
          ...group,
          [activeListType]: [
            ...(group[activeListType] as AdvancedBlockingUrlEntry[]),
            trimmed,
          ],
        };
      });
  ...
  const addBulkUrlsToAllGroups = useCallback(
    async (urlsText: string) => {
      ...
      if (skippedCount > 0) {
        setError(
          `Added ${addedCount} URLs, skipped ${skippedCount} duplicates`,
        );
      }
    },
    [draftConfig, selectedGroups, activeListType, getGroupUrls, cloneConfig],
  ...
  const removeUrlFromAllGroups = useCallback(
    async (url: string) => {
      ...
      updatedConfig.groups = updatedConfig.groups.map((group) => {
        ...
        return { ...group, [activeListType]: filtered };
      });
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    const urlExists = targetGroups.some((group) => {
      return (group[activeListType] as AdvancedBlockingUrlEntry[]).some(
        (entry) => {
          const entryUrl = typeof entry === "string" ? entry : entry.url;
    ...
    updatedConfig.groups = updatedConfig.groups.map((group) => {
      ...
      const updated = (group[activeListType] as AdvancedBlockingUrlEntry[]).map(
        (entry) => {
          const entryUrl = typeof entry === "string" ? entry : entry.url;
          if (entryUrl === oldUrl) {
            // Preserve entry structure (string or object)
            return typeof entry === "string" ? newUrl : (
                { ...entry, url: newUrl }
              );
          }
          return entry;
      ...
      return { ...group, [activeListType]: updated };
    });
  ...
  return (
    <section className="configuration-editor configuration-editor--stacked">
      ...
      : <>
          ...
          <div className="group-management__section">
            ...
            <div className="field-group">
              <div
                className="field-group__control"
                style={{ maxWidth: "300px" }}
              >
                <label htmlFor={refreshIntervalId}>
                  List source refresh interval
                </label>
                <div
                ...
                >
                    ...
                    onChange={(event) => {
                      ...
                      setDraftConfig((prev) => {
                        ...
                        if (!raw) {
                          return {
                            ...prev,
                            blockListUrlUpdateIntervalHours: undefined,
                          };
                        }
                        ...
                        return {
                          ...prev,
                          blockListUrlUpdateIntervalHours: next,
                        };
                      });
                    }}
                  ...
                  <span>hours</span>
                  <input
                    id={refreshIntervalMinutesId}
                    name="listRefreshMinutes"
                    type="number"
                    min="0"
                    max="59"
                    style={{ width: "55px" }}
                    value={
                      isRefreshMinutesFocused ?
                        (draftConfig?.blockListUrlUpdateIntervalMinutes ?? "")
                      : (draftConfig?.blockListUrlUpdateIntervalMinutes ?? 0)
                    }
                    onChange={(event) => {
                      const raw = event.target.value;
                      setDraftConfig((prev) => {
                        ...
                        const clamped = Math.max(
                          0,
                          Math.min(59, Math.trunc(next)),
                        );

                        return {
                          ...prev,
                          blockListUrlUpdateIntervalMinutes: clamped,
                        };
                      });
                    }}
                    onFocus={() => setIsRefreshMinutesFocused(true)}
                    onBlur={() => {
                      setIsRefreshMinutesFocused(false);
                      setDraftConfig((prev) => {
                        if (!prev) {
                          return prev;
                        }
                        ...
                        if (
                          prev.blockListUrlUpdateIntervalMinutes !== undefined
                        ) {
                          return prev;
                        }
          ...
          <div className="multi-group-editor__group-selection">
            ...
            <div className="multi-group-editor__groups">
              {groups.map((group) => (
                <label
                  key={group.name}
                  className="multi-group-editor__group-checkbox"
                >
                  <input
                    type="checkbox"
            ...
            <div className="list-source-editor__type-buttons">
              {LIST_TYPES.map((listType) => (
                <button
                  key={listType.key}
                  type="button"
                  className={`list-source-editor__type-button ${
                    activeListType === listType.key ?
                      "list-source-editor__type-button--active"
                    : ""
                  }`}
          ...
          {selectedGroups.size === 0 ?
            <div className="multi-group-editor__placeholder">
              <p>
                Select at least one group to view and manage{" "}
                {activeListConfig.label.toLowerCase()}.
              </p>
            </div>
          : <>
              {/* Search Filter */}
              ...
              <div className="multi-group-editor__section">
                    ...
                    <div className="multi-group-editor__selection-controls">
                      <button
                        ...
                        disabled={
                          disabled ||
                          saving ||
                          selectedUrls.size === filteredIntersection.length
                        }
                      >
                        Select All
                      </button>
                      ...
                      {selectedUrls.size > 0 && (
                        <button
                          ...
                          onClick={() =>
                            setUrlsToDelete(Array.from(selectedUrls))
                          }
                          disabled={disabled || saving}
                        >
                ...
                : <ul className="multi-group-editor__url-list">
                    {filteredIntersection.map((url) => {
                      const isEditing =
                        editingUrl?.url === url && !editingUrl?.groupName;
                      return (
                        <li
                          key={url}
                          className={`multi-group-editor__url-item ${
                            isEditing ?
                              "multi-group-editor__url-item--editing"
                            : ""
                          }`}
                        >
                          {isEditing ?
              ...
              {Object.keys(filteredDifferences).length > 0 && (
                <div className="multi-group-editor__section">
                  ...
                  {Object.entries(filteredDifferences).map(
                    ([groupName, urls]) => (
                      <div
                        key={groupName}
                        className="multi-group-editor__difference-group"
                      >
                        ...
                        <ul className="multi-group-editor__url-list">
                          {urls.map((url) => {
                            const isEditing =
                              editingUrl?.url === url &&
                              editingUrl?.groupName === groupName;
                            return (
                              <li
                                key={url}
                                className={`multi-group-editor__url-item ${
              ...
              <div className="multi-group-editor__add-form">
                ...
                >
                  <div className="multi-group-editor__add-input-group">
                    ...
                    >
                      {bulkMode ?
                        `Add All to ${selectedGroups.size} Group${selectedGroups.size === 1 ? "" : "s"}`
                  ...
                  {activeListConfig.hint && !bulkMode && (
                    <p className="multi-group-editor__hint">
                      {activeListConfig.hint}
                    </p>
                  )}
                  {bulkMode && (
                    <p className="multi-group-editor__hint">
                      Paste one URL per line. Duplicates will be automatically
                      skipped.
                    </p>
                  )}
                </form>
      ...
      {urlToRemove && (
        <div
          className="multi-group-editor__dialog-overlay"
        ...
        >
          ...
          >
            ...
            <div className="multi-group-editor__dialog-body">
              <p>
                Are you sure you want to remove this URL from all{" "}
                {selectedGroups.size} selected group
                {selectedGroups.size === 1 ? "" : "s"}?
              </p>
          ...
          >
            ...
            <div className="multi-group-editor__dialog-body">
              <p>
                ...
                <strong>
                  {urlsToDelete.length} URL
                  {urlsToDelete.length === 1 ? "" : "s"}
                </strong>{" "}
                from all {selectedGroups.size} selected group
                {selectedGroups.size === 1 ? "" : "s"}?
              </p>
              ...
              {urlsToDelete.length <= 10 && (
                <div className="multi-group-editor__dialog-url-list">
                  ...
                  {urlsToDelete.map((url) => (
                    <div
                      key={url}
                      className="multi-group-editor__dialog-url-item"
                    >
                      <UrlDisplay url={url} />
                    </div>
                  ))}
            ...
            <div className="multi-group-editor__dialog-actions">
              ...
              >
                {saving ?
                  "Deleting..."
                : `Delete ${urlsToDelete.length} URL${urlsToDelete.length === 1 ? "" : "s"}`
                }
              </button>
            </div>
          </div>
