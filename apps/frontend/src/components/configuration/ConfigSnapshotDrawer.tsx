...
const methodLabel = (method: ConfigSnapshotMethod): string => {
  return (
    method === "built-in" ? "Built-in"
    : method === "rule-optimizer" ? "Rule Optimizer"
    : "Advanced"
  );
};

const historyTitle = (method: ConfigSnapshotMethod): string => {
  return method === "rule-optimizer" ?
      "DNS Rule Optimizer History"
    : "DNS Filtering History";
};

const methodPillLabel = (method: ConfigSnapshotMethod): string => {
  return method === "rule-optimizer" ?
      "Rule Optimizer context"
    : `${methodLabel(method)} method`;
};

const stateLabel = (method: ConfigSnapshotMethod): string => {
  return method === "rule-optimizer" ?
      "DNS Rule Optimizer state"
    : "DNS filtering state";
};

const emptyStateCopy = (method: ConfigSnapshotMethod): string => {
  return method === "rule-optimizer" ?
      "Capture the current DNS Rule Optimizer state to enable quick rollback. Pinned snapshots never expire during retention cleanup."
    : "Capture the current DNS filtering state to enable quick rollback. Pinned snapshots never expire during retention cleanup.";
};

const originLabel = (origin: ConfigSnapshotMetadata["origin"]): string => {
  return (
    origin === "automatic" ? "Auto"
    : origin === "rule-optimization" ? "Rule Optimizer"
    : "Manual"
  );
};
...
const summarizeCounts = (
  ...
  meta?: ConfigSnapshotMetadata | null,
...
export const ConfigSnapshotDrawer: React.FC<ConfigSnapshotDrawerProps> = ({
  ...
  const [snapshots, setSnapshots] = useState<ConfigSnapshotMetadata[]>([]);
  ...
  const startEditNote = (snapshot: ConfigSnapshotMetadata) => {
        ...
        {nodeId && (
          <>
                  ...
                  <div className="snapshot-drawer__dialog-header">
                    <div>
                      ...
                      <p className="snapshot-drawer__dialog-subtext">
                        ...
                        {methodLabel(method)} {stateLabel(method)} on this node.
                      </p>
                    </div>
                    <button
                      type="button"
                    ...
                    <div className="snapshot-drawer__header-text">
                      ...
                      <div className="snapshot-drawer__dialog-meta">
                        {(() => {
                          ...
                          if (!meta) return null;
                          const counts = summarizeCounts(viewedSnapshot, meta);
                          return (
                            <>
                              <span>
                      ...
                      <div className="snapshot-drawer__note-card">
                        ...
                        <div>
                          {(
                            ...
                            renderExpandableNote(viewedSnapshot.metadata.note, {
                              expanded: expandedViewNote,
                              onToggle: () =>
                                setExpandedViewNote((previous) => !previous),
                              previewLines: 8,
                              previewChars: 320,
                            })
                          : <span>No note</span>}
            ...
            <SnapshotDrawerActions
              ...
              primaryDisabled={loading || creating}
            ...
            {!loading && !error && sortedSnapshots.length === 0 && (
              <SnapshotDrawerEmptyState message={emptyStateCopy(method)} />
            )}
            ...
            {!loading && !error && sortedSnapshots.length > 0 && (
              <div className="snapshot-drawer__list">
                ...
                {sortedSnapshots.map((snap) => {
                  ...
                  const origin = originLabel(snap.origin);
                        ...
                        <div className="snapshot-drawer__item-main">
                          <div className="snapshot-drawer__item-meta">
                            <span className="snapshot-drawer__time-primary">
                              {formatSnapshotRelative(snap.createdAt)}
                            </span>
