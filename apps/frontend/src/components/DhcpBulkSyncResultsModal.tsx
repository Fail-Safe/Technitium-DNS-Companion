...
import { faCheck, faExclamationTriangle, faXmark, faBan } from '@fortawesome/free-solid-svg-icons';
 ...
 */
export const DhcpBulkSyncResultsModal: React.FC<DhcpBulkSyncResultsModalProps> = ({
    isOpen,
    result,
    onClose,
    onRetry,
        ...
        <div className="modal-overlay" onClick={onClose}>
            ...
            >
                ...
                <div className="modal__body">
                    {/* Summary Section */}
                    <div className={`dhcp-bulk-sync-results__summary dhcp-bulk-sync-results__summary--${overallStatus}`}>
                        <div className="dhcp-bulk-sync-results__summary-icon">
                            {allSuccess && <FontAwesomeIcon icon={faCheck} />}
                            {partialSuccess && <FontAwesomeIcon icon={faExclamationTriangle} />}
                    ...
                    <div className="dhcp-bulk-sync-results__stats">
                        ...
                        {result.totalSkipped > 0 && (
                            <div className="dhcp-bulk-sync-results__stat dhcp-bulk-sync-results__stat--skipped">
                                ...
                                <div className="dhcp-bulk-sync-results__stat-label">Skipped</div>
                            </div>
                        )}
                    ...
                    <div className="dhcp-bulk-sync-results__nodes">
                        <h3 className="dhcp-bulk-sync-results__section-title">Node Details</h3>
                        {result.nodeResults.map((nodeResult) => (
                            ...
                            >
                                ...
                                {/* Per-Scope Results */}
                                {nodeResult.scopeResults && nodeResult.scopeResults.length > 0 && (
                                        ...
                                        {nodeResult.scopeResults.map((scopeResult, idx) => (
                                            ...
                                            >
                                                <span className="dhcp-bulk-sync-results__scope-icon">
                                                    {scopeResult.status === 'synced' && <FontAwesomeIcon icon={faCheck} />}
                                                    {scopeResult.status === 'skipped' && <FontAwesomeIcon icon={faBan} />}
                                                    {scopeResult.status === 'failed' && <FontAwesomeIcon icon={faXmark} />}
                                                </span>
