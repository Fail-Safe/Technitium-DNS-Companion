...
interface UseBlockListCatalogReturn {
  ...
  ) => Promise<HageziUpdateCheckResult | null>;
  /** Compare current URLs with latest Hagezi Catalog */
  compareWithHagezi: (
    currentUrls: string[],
  ) => Promise<HageziCompareResult | null>;
...
export function useBlockListCatalog(): UseBlockListCatalogReturn {
  ...
  const fetchHageziCatalog = useCallback(
    async (forceRefresh = false): Promise<HageziUpdateCheckResult | null> => {
      setIsLoading(true);
      setError(null);
  ...
  const compareWithHagezi = useCallback(
    async (currentUrls: string[]): Promise<HageziCompareResult | null> => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to compare: ${response.status} ${response.statusText}`,
          );
        }
  ...
  return { fetchHageziCatalog, compareWithHagezi, isLoading, error, catalog };
}
