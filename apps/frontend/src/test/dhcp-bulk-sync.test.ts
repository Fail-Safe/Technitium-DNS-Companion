...
describe("DHCP Bulk Sync - Business Logic", () => {
  describe("Sync Strategy Validation", () => {
    it("should validate skip-existing strategy", () => {
      ...
      expect(["skip-existing", "overwrite-all", "merge-missing"]).toContain(
        request.strategy,
      );
    });
  ...
  describe("Response Processing", () => {
    ...
    it("should handle partial sync result", () => {
      ...
      expect(result.nodeResults[0].scopeResults[1].error).toBe(
        "Connection timeout",
      );
    });
    ...
    it("should handle skipped scopes", () => {
        ...
        nodeResults: [
          {
            ...
            scopeResults: [
              { scopeName: "default", status: "synced" },
              {
                ...
                status: "skipped",
                reason:
                  "Scope already exists on target (skip-existing strategy)",
                differences: ["Ping check enabled: true → false"],
              },
            ],
            syncedCount: 1,
            skippedCount: 1,
      ...
      expect(result.nodeResults[0].scopeResults[1].reason).toContain(
        "already exists",
      );
      expect(result.nodeResults[0].scopeResults[1].differences?.[0]).toContain(
        "Ping check enabled",
      );
    });
  ...
  describe("Summary Calculations", () => {
    it("should calculate summary for single target", () => {
      ...
      const total =
        nodeResult.syncedCount +
        nodeResult.skippedCount +
        nodeResult.failedCount;
      expect(total).toBe(3);
  ...
  describe("Scope Filtering Logic", () => {
    it("should filter scopes when scopeNames provided", () => {
      ...
      const filtered = availableScopes.filter((scope) =>
        filterList.some(
          (name: string) => name.toLowerCase() === scope.name.toLowerCase(),
        ),
      );
  ...
  describe("Strategy Behavior Prediction", () => {
    it("should predict skip-existing behavior", () => {
      ...
      const willSync = sourceScopes.filter(
        (scope) => !targetScopes.includes(scope),
      );
      const willSkip = sourceScopes.filter((scope) =>
...
describe("DHCP Bulk Sync - Error Handling", () => {
  it("should handle network errors gracefully", () => {
    const error = new Error(
      "Failed to bulk sync DHCP scopes (500): Internal Server Error",
    );
